...
const authSlice = createSlice({
  ...
  extraReducers: (builder: any) => {
    ...
    // Handle auth hydration thunk
    builder
      .addCase("auth/hydrate/pending", (state: AuthState) => {
        state.status = "loading";
        state.error = null;
      })
      .addCase(
      ...
      .addCase("auth/hydrate/rejected", (state: AuthState, action: any) => {
        state.status = "unauthenticated";
        state.user = null;
        state.accessToken = null;
        state.refreshToken = null;
        state.error = (action.payload as string) || "Auth hydration failed";
      });
  },
});

export const { clearError, logout, setCredentials, updateUser } =
  authSlice.actions;
